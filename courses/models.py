...
def proposal_rename_video(instance, filename):
    '''instance here is the user of the profile class '''
    global vid 
    upload_to = 'video/'
    vid=vid+1
    ...
    for elm in list:
        ...
        if elm==extension:
            filename = "proposal/course_video/video-.{}".format(extension)
            return os.path.join(upload_to,  filename)
    return print('format error')
...
def proposal_rename_file(instance, filename):
    global fil
    '''instance here is the user of the profile class '''
    upload_to = 'file/'
    # recuperer l'extension de l'image et traiter
    ...
    for elm in list:
        fil = fil+1
        if elm==extension:
            filename = "proposal/course_file/file-{}.{}".format(fil, extension)
            return os.path.join(upload_to,  filename)
    return print('format error')
...
def rename_present_file(instance, filename):
    ...
    for elm in list:
        fil_pre = fil_pre+1
        if elm==extension:
            filename = "proposal/course_present_file/file-{}.{}".format(fil_pre, extension)
            return os.path.join(upload_to,  filename)
    return print('format error')

def proposal_rename_present_file(instance, filename):
    global fil_pre
    '''instance here is the user of the profile class '''
    upload_to = 'file/proposal'
    # recuperer l'extension de l'image et traiter
    #img.png
    extension = filename.split('.')[-1]
    list = ['doc', 'pdf', 'docx']
...
class ModuleProposal(models.Model):
    ...
    description = models.TextField(max_length=500)
    position = models.PositiveSmallIntegerField(verbose_name= 'chapitre_no') # verbose name(cmt appele ce champ)
    video = models.FileField(upload_to=proposal_rename_video,
                             null=True, blank=True, verbose_name="video course")
    present_file = models.FileField(upload_to=proposal_rename_file, null=True, blank=True, verbose_name="presenting file")
    pdf_course = models.FileField(upload_to=proposal_rename_file, null=True, blank=True)
    created = models.DateTimeField(auto_now_add=True, blank=True, null=True)
    created = models.DateTimeField(auto_now_add=True)
